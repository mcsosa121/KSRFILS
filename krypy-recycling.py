...
from krypy.deflation import DeflatedCg, DeflatedGmres
...
ts = time.time()
for i in range(0,len(Asys)):
    U=find_deflation_subspace(Asys[i],b,10)
...
vector_factory = RitzFactorySimple(n_vectors=10, which='sm')
ts = time.time()
...
ts = time.time()
U=find_deflation_subspace(Asys[0],b,10)
recycled_sol.append(DeflatedCg(systems[0],U=U,maxiter=1000,store_arnoldi=True))
for i in range(1,len(Asys)):
    print(i)
    recycled_sol.append(DeflatedCg(systems[i],U=reuse_deflation_subspace(recycled_sol[i-1]),maxiter=1000,store_arnoldi=True))
te = time.time()
print("Recycled CG (creates multiple subspaces) time taken: ", (te-ts)*1000)
'''
